...
init <- function(pmwgs, start_mu = NULL, start_var = NULL,
  ...
  if (is.null(start_var)) {
    ...
    start_var_group <- MCMCpack::riwish(pmwgs$n_pars * 3,diag(pmwgs$n_pars))
  }
  
  if(useC){
    #Using C sometimes doesn't work depending on your setup, so if you get errors set useC to F
  ...
  pmwgs$samples$last_group_var_inv <-array(rep(MASS::ginv(start_var), pmwgs$n_groups), 
...
gibbs_step_pop <- function(sampler){
  ...
  var_mu <- MASS::ginv(sampler$n_groups * last$tvinv + prior$theta_mu_invar)
  ...
  tvar <- MCMCpack::riwish(hyper$v_half + sampler$n_pars - 1 + sampler$n_groups, B_half) # New sample for group variance
...
gibbs_step_group <- function(group, sampler){
  ...
  var_mu <- MASS::ginv(n_subjects * last$tvinv + last$theta_mu_invar)
  ...
  cov_temp <- (theta_temp) %*% (t(theta_temp))
  B_half <- 2 * hyper$v_half * diag(1 / last$a_half) + cov_temp # nolint
  tvar <- MCMCpack::riwish(hyper$v_half + sampler$n_pars - 1 + n_subjects, B_half) # New sample for group variance
  tvinv <- MASS::ginv(tvar)
...
new_particle <- function (s, data, num_particles, group_parameters, eff_mu = NULL, 
  ...
  lg <- dmv(x = proposals, mean = group_mu, sigma = group_var, log = TRUE)
  prop_density <- dmv(x = proposals, mean = subj_mu, sigma = group_var * (epsilon[s]^2))
  if (mix_proportion[3] == 0) {
    eff_density <- 0
  }
  else {
...
test_sampler_adapted <- function(pmwgs, n_unique, i, n_cores) {
  ...
  if (all(n_unique_sub > n_unique)) {
    ...
    attempt <- tryCatch({
      if(n_cores > 1){
        mclapply(X = 1:pmwgs$n_subjects,FUN = get_conditionals,samples = test_samples, n_pars, i, 
                 pmwgs$subjectgroups, mc.cores = n_cores)
      } else{
